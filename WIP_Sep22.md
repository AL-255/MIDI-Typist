...
The connected M1 runs the clean wireless-enabled build `b3410b1`. A Bluetooth
pairing status gap no longer stops the application: output is gated after
500 ms without status, while a longer bounded pairing watchdog remains active.
Bluetooth slot 2 paired and delivered an A key-down/up pair on its own host HID
device after the status gap. Linked-ARM wireless and runtime-power tests pass.
...
wireless mode. This does not establish Bluetooth slot 3, sleep/wake, battery
behavior, or long-term scan timing. The factory updater
...
Next checkpoint: exercise Bluetooth slot 3 and physical power/cable transitions
before broadening release claims.
