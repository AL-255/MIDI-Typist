...
static void gui_snapshot(uint32_t now)
{
    ...
    s_last_gui = now;
    _Static_assert(SCAN_STREAM_GUI_SIZE==KEYBOARD_TELEMETRY_SIZE,"GUI transport payload size");
    const keyboard_telemetry_t status={
        .sequence=s_gui_sequence, .ack=s_gui_ack, .result=s_gui_result,
    ...
    uint8_t out[KEYBOARD_TELEMETRY_SIZE];
    if(keyboard_telemetry_encode(out,&s_app,&s_cal_store,&status,now)) {
        ++s_gui_sequence;
