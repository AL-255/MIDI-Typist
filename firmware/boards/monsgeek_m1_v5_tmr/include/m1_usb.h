...
enum { M1_USB_HID_IN=0x81, M1_USB_MIDI_IN=0x82, M1_USB_MIDI_OUT=2,
       M1_USB_CONSUMER_IN=0x83, M1_USB_CONSUMER_INTERFACE=3,
       M1_USB_CONSUMER_USAGE_MAX=0x03ff,
       M1_USB_FS_PACKET=64, M1_USB_HS_PACKET=512, M1_USB_INTERFACES=4 };
...
/* Independent Consumer Control interface: one u16 usage, zero releases it.
 * Copy-on-acceptance, with the same idle/epoch rules as keyboard HID. */
...
usbd_desc_t *m1_usb_consumer_report_descriptor(void);
usbd_desc_t *m1_usb_consumer_hid_descriptor(void);
