...
_Static_assert(sizeof(fs)==166 && sizeof(hs)==166,"configuration descriptor length");
...
DESC(lang) DESC(maker) DESC(product) DESC(performance) DESC(control) DESC(report)
