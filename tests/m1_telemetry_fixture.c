...
int main(void)
{
    ...
    assert(midi_control_ready());
    receive(MT_COMMAND,"calibration read",1);
    assert(midi_sysex_decode(wire,used,&info,payload,sizeof(payload)));
    assert(info.kind==MT_ACK && info.sequence==1 && info.length==MT_BOUNDS_SIZE(82));
    assert(!memcmp(payload,"MTB1",4) && payload[6]==82 && payload[7]==3);
    unsigned last=MT_BOUNDS_HEADER_SIZE+81u*MT_BOUNDS_RECORD_SIZE;
    assert((payload[last]|(unsigned)payload[last+1]<<8)==3981);
    receive(MT_COMMAND,"cfg set 7 81 2500 2700",2);
    assert(status.ack==7 && status.result==1 && raw.press[81]==2500);
    receive(MT_COMMAND,"cfg midi 8 81 60",3);
    assert(status.ack==8 && status.result==1 && midi.mapping[81]==60);
    receive(MT_COMMAND,"cfg key 9 81 135",4);
