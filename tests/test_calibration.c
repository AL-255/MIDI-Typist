...
static keyboard_calibration_t capture(unsigned profile, uint32_t now)
{
    ...
    assert(!calibration_start(&c,profile,n,now));
    calibration_frame(&c,raw,NULL,NULL,true,false,now+100);
    assert(c.state==CAL_RELEASE);
    calibration_frame(&c,raw,NULL,NULL,true,true,now+200);
    calibration_frame(&c,raw,NULL,NULL,true,true,now+699);
    assert(c.state==CAL_SETTLE);
    calibration_frame(&c,raw,NULL,NULL,true,true,now+700);
    assert(c.state==CAL_COLLECT);
    now+=701;
    for (unsigned i=0;i<n;++i) {
        raw[i]=1000+i;
        calibration_frame(&c,raw,NULL,NULL,true,false,now);
        assert(c.selected==i);
        calibration_frame(&c,raw,NULL,NULL,true,false,now+999);
        assert(c.completed==i);
        calibration_frame(&c,raw,NULL,NULL,true,false,now+1000);
        assert(c.completed==i+1 && c.lower[i]==1000+i && c.upper[i]==4000);
        raw[i]=4000;
        calibration_frame(&c,raw,NULL,NULL,true,true,now+1001);
        now+=1100;
    }
...
static void parallel(unsigned profile, uint32_t now)
{
    ...
    uint16_t raw[65]; fill(raw,n,4000);
    assert(calibration_start(&c,profile,n,now));
    calibration_frame(&c,raw,NULL,NULL,true,true,now);
    calibration_frame(&c,raw,NULL,NULL,true,true,now+500);
    for (unsigned i=0;i<n;++i) raw[i]=1000+i;
    calibration_frame(&c,raw,NULL,NULL,true,false,now+501);
    ...
    for (unsigned sample=1;sample<=8000;++sample) {
        ...
        calibration_frame(&c,raw,NULL,NULL,true,false,now+501+sample/8);
...
int main(void)
{
    ...
    calibration_frame(&s,raw,NULL,NULL,true,true,0); calibration_frame(&s,raw,NULL,NULL,true,true,500);
    raw[0]=1000; raw[1]=1100; raw[2]=1200;
    calibration_frame(&s,raw,NULL,NULL,true,false,501);
    assert(s.selected==0 && !s.completed && s.holds[0].active && s.holds[1].active && s.holds[2].active);
    raw[2]=4000; calibration_frame(&s,raw,NULL,NULL,true,false,502);
    assert(!s.holds[2].active && s.holds[1].active);
    raw[0]=1500; calibration_frame(&s,raw,NULL,NULL,true,false,1000);
    calibration_frame(&s,raw,NULL,NULL,true,false,1502); assert(s.completed==1 && s.lower[1]==1100 && s.holds[0].active);
    calibration_frame(&s,raw,NULL,NULL,true,false,2000); assert(s.completed==2 && s.lower[0]==1500);
    /* Keep completed keys held while another starts and completes. */
    raw[2]=1250; calibration_frame(&s,raw,NULL,NULL,true,false,2001);
    calibration_frame(&s,raw,NULL,NULL,true,false,3000); assert(s.completed==2);
    calibration_frame(&s,raw,NULL,NULL,true,false,3001); assert(s.completed==3 && s.lower[2]==1250);
    ...
    calibration_frame(&s,raw,NULL,NULL,true,true,0); assert(s.reason==CAL_INVALID);
